...
export class ChallengesController {
  ...
  async getChallenges(
    ...
    try {
      if (data.playerId) {
        return await this.challengeService.getChallengeOfPlayer(data.playerId);
      } else if (data.challengeId) {
        return await this.challengeService.getChallengeById(data.challengeId);
      }
  ...
  async updateGameChallenge(@Payload() data: any, @Ctx() context: RmqContext) {
    const channel = context.getChannelRef();
    const originalMessage = context.getMessage();
